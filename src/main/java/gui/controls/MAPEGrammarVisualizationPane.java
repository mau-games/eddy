...
public class MAPEGrammarVisualizationPane extends BorderPane implements Listener
{
	...
	public MAPEGrammarVisualizationPane()
	{
		super();
		EventRouter.getInstance().registerListener(this, new SaveDisplayedCells());
	}
	
	public void init(ArrayList<SuggestionNarrativeStructure> narrativeDisplays, String xLabelText, String yLabelText, int width, int height)
	{
...
//		mapeScroll.setPrefWidth(50);
		...
		SetupInnerGrid(narrativeDisplays, width, height);
...
//		BorderPane.setAlignment(innerSuggestions, Pos.CENTER_RIGHT);
	...
	public void dimensionsUpdated(ArrayList<SuggestionNarrativeStructure> narrativeDisplays, MAPEDimensionGrammarFXML[] dimensions)
	{
		...
		SetupInnerGrid(narrativeDisplays, dimensions[0].getGranularity(),  dimensions[1].getGranularity());
	}
	...
	public void SetupInnerGrid(ArrayList<SuggestionNarrativeStructure> narrativeDisplays, int xResolution, int yResolution)
	{
		...
		for(int j = 0, red = yResolution; j < yResolution; j++, red--)
		{
			...
			{
				GridPane.clearConstraints(narrativeDisplays.get((i - 1) + j * xResolution).getNarrativeGraphicNode());
				GridPane.setConstraints(narrativeDisplays.get((i - 1) + j * xResolution).getNarrativeGraphicNode(), i, (red - 1));
				innerSuggestions.getChildren().add(narrativeDisplays.get((i - 1) + j * xResolution).getNarrativeGraphicNode());
			}
		}
