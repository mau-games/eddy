...
public class WorldViewController extends BorderPane implements Listener
{
	...
	private Button tropetwistBtn = new Button();
	...
	private Button scaleBtn = new Button();
...
//			ani.getView().setFitHeight(50);
		...
		arrangeControls(scaleBtn, 0, 400, 300, 50);
...
//		arrangeControls(mView, 0, 200, 50, 50);
		...
		restartProgram.setTranslateY(200);
...
//		widthLabel.setTextFill(Paint);
		...
		buttonPane.getChildren().add(scaleBtn);
...
//		removeRoomBtn.setText("REMOVE ROOM");
		...
		tropetwistBtn.setText("TROPETWIST VIEW");
...
//        		}
		...
		tropetwistBtn.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e)
			{
				if(dungeon.getSelectedRoom().getDoorCount() > 0)
...
//		
		...
		scaleBtn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				if(dungeon.getSelectedRoom() != null)
				{
					Room scaleRoom = dungeon.getSelectedRoom();
					router.postEvent(new RequestScaleRoom(dungeon.getSelectedRoom(), scaleRoom.getRoomHeight(), scaleRoom.getRoomWidth() ));
				}
			}
		});
	}
