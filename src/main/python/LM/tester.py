from transformers import GPT2Tokenizer, GPTNeoForCausalLM, pipeline, TrainingArguments, Trainer, DataCollatorForLanguageModeling
...
def tokenize_training_function(examples):
    num_ent = len(examples["id"])
    inputs = []
    for i in range(num_ent):
        num_in = len(examples["inputs"][i])
        inputs.append(examples["output"][i])
        for j in range(num_in):
            inputs.append(examples["inputs"][i]["input"][j])
    result = ModelData.tokenizer(inputs, padding="max_length", truncation=True)
    return result
...
def train():
    training_encoding = ModelData.dataset.map(tokenize_training_function, batched=True, remove_columns=["id", "inputs", "output"])
    training_dataset = training_encoding.map(
        concatenate,
        batched=True,
        batch_size=1000
    )
    validation_encoding = ModelData.dataset.map(tokenize_validation_function, batched=True, remove_columns=["id", "inputs", "output"])
    validation_dataset = validation_encoding.map(
        concatenate,
        batched=True,
        batch_size=1000
    )
    ModelData.tokenizer.decode(training_dataset["train"][1]["input_ids"])
    ModelData.tokenizer.decode(validation_dataset["validation"][1]["input_ids"])
    training_dataset = training_dataset.with_format("torch")
    validation_dataset = validation_dataset.with_format("torch")
    ModelData.validation_dataset = validation_dataset

    data_collator = DataCollatorForLanguageModeling(tokenizer=ModelData.tokenizer, mlm=False)
...
def init():
    ...
    # Terminal command to regenerate dataset_infos.json
    # datasets-cli test datasets/nardat/nardat.py --save_infos --all_configs

    base_url = "https://github.com/Hahoolah/nardat/blob/main/"
    # r = requests.get('https://raw.githubusercontent.com/Hahoolah/nardat/main/train.json')
    # with open("./datasets/nardat/train.json", 'w', encoding="utf-8") as file:
    #     file.write(r.text)
    # r = requests.get('https://raw.githubusercontent.com/Hahoolah/nardat/main/validate.json')
    # with open("./datasets/nardat/validate.json", 'w', encoding="utf-8") as file:
    #     file.write(r.text)
    ...
    #     file.write(r.text)
    ModelData.dataset = load_dataset("./nardat")
...
if ModelData.enable_model:
    ...
    for i in range(ModelData.num_runs):
        ...
        train()
        end_time = time.perf_counter()
        print("Training iteration " + str(i + 1) + f" completed after {end_time - start_time:0.2f} sec.\n")
